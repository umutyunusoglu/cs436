# AWS Serverless Deployment Guide: Price Tracker (Multi-AZ)
...
| NAT Gateway | 1 AZ, ~$0.048/hr + data processing | ~$36.00 |
| Elastic IP | Attached to NAT Gateway | ~$3.60 |
...
> **Credit runway:** With $100 in AWS credits, this architecture runs for approximately **42 days** before out-of-pocket billing begins.
>
> **Cost trade-off note:** A single NAT Gateway is deployed in AZ-a only (saving ~$36/month vs. two gateways). If AZ-a experiences an outage, Lambdas in AZ-b lose outbound internet access. This is an acceptable risk for a course project. Document it in your report under "Cost vs. Availability Trade-offs."
>
...
*We configure a 2-AZ network footprint with one NAT Gateway for cost efficiency.*
...
AWS will automatically wire the Internet Gateway, subnets, and route tables.
...
- **VPC:** Select `price-tracker-vpc`
- **Subnets:** Add the **two private subnets** associated with your VPC.
- Click **Create**.
...
3. Select `price-tracker-vpc`, a **private subnet** *(crucial — this routes through the NAT Gateway, which provides the internet access needed for the installation step below)*, and `lambda-sg`.
...
- **Advanced settings:** Enable VPC → select `price-tracker-vpc`, both private subnets, `lambda-sg`
...
> ⚠️ This architecture costs approximately **$71/month**. Execute this sequence promptly when the project is complete to stop all charges. Follow the order exactly — deleting resources out of sequence causes dependency errors.
...
11. **VPC Network** *(in this order — the VPC cannot be deleted while the NAT Gateway exists):*
    - Go to **NAT Gateways** → select the NAT Gateway → **Delete**.
    - Go to **Elastic IPs** → select the IP that was associated with the NAT Gateway → **Release**.
    - Go to **VPCs** → select `price-tracker-vpc` → **Delete VPC**. This automatically removes subnets, route tables, the Internet Gateway association, and security groups.
...
Internet
  ...
  │                                                     │
  └─── Tiingo API ◄── outbound (NAT GW) ───────────────┤
                                                        │
                              ┌─────────────────────────┤
                              │   VPC / Private Subnets  │
                              │                          │
                              │  ┌──────────────────┐    │
                              │  │  price-fetcher   │◄───┤ EventBridge (5 min)
                              │  │  model-trainer   │◄───┤ EventBridge (weekly)
                              │  │  model-invoker   │    │
                              │  │  api-handler     │◄───┘
                              │  └────────┬─────────┘
                              │           │ port 5432
                              ...
                              │  └──────────────────────────────────┘
                              │
                              │  S3: pricetracker-models (ML artifacts)
...
*Guide version: final — incorporates all architectural, security, networking, operational, and teardown refinements identified across review iterations.*
